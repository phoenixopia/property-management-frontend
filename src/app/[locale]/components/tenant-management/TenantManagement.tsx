...
const TenantManagement = () => {
    ...
    const [addUser, setUserTenant] = useState(false);
    ...
    const { data: userData, isPending, isError, isSuccess } = useQuery({
        queryKey: ["getAllTenantData", currentPage, searchQuery],
        queryFn: () => getAllTenants(currentPage, searchQuery),
    });
    ...
    const openAddUserModal = () => setUserTenant(true);
    const openEditUserModal = (user: User) => {
        setSelectedUser(user);
        setEditUser(true);
    };
    const closeEditUserPage = () => setEditUser(false);
    const closeAddUserPage = () => setUserTenant(false);
    ...
    const deactivateMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: ['getAllTenantData'] });
        },
        onError: () => {
    ...
    const exportMutation = useMutation({
        mutationFn: async () => {
          const response = await exportAllTenants(currentPage, searchQuery);
                                ...
                                usersList?.map((user: User) => (
                                    <tr key={user.id} className='bg-white capitalize text-gray-500 dark:text-gray-200 border-b dark:bg-[#333538] dark:border-gray-700 border-gray-200'>
                                         ...
                                         */}
                                         ...
                                         <td className='px-6 py-4 gap-1'>{user?.groups.map((groupList,keys)=><p key={keys}>{groupList}</p>)}</td>
