...
const Logs = () => {
    ...
    const [currentlogsData, setCurrentlogsData] = useState<User | null>(null);
    ...
    const { data: logsData, isPending, isError, isSuccess } = useQuery({
        queryKey: ["getAllLogs", currentPage],
        queryFn: () => getAllLogs(currentPage),
    });
   
    const logsList = logsData?.data || [];
    const total_pages = logsData?.total_pages || 0;
    const previous = logsData?.previous || false;
    const next = logsData?.next || false;
    ...
    const seenMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: ['shortlogsData'] });
    ...
    const viewNotiData = (notification: any) => {
        ...
        setCurrentlogsData(notification);
        seenMutation.mutate(notification?.id)
    };

    console.log(logsList,'data of the logs after the fetch')
        ...
        <div className='flex flex-col justify-between p-4'>
            ...
            <div className='py-5'>
                        ...
                        <tbody>
                            {isPending ? (
                                ...
                                logsList?.map((log: any) => (
                                    <tr key={log.id} className='bg-white capitalize text-gray-500 dark:text-gray-200 border-b dark:bg-[#333538] dark:border-gray-700 border-gray-200'>
                                        <td className='px-6 py-4'>{log?.remote_addr || '-'}</td>
                                        <td className='px-6 py-4'>{log?.action_display} {log?.object_repr}</td>
                                        <td className='px-6 py-4'>{log?.timestamp} <br/>{formatTimeAgo(log?.timestamp)}</td>
                ...
                <div className='flex w-full justify-between my-2 p-4 flex-col xl:flex-row items-center gap-3 xl:gap-0 shadow-sm dark:bg-[#333538]'>
                    <div>
                        <p className='text-gray-600 dark:text-gray-200'>Showing {logsList.length} of {count} entries</p>
                    </div>
            ...
            {/* {currentNotificationView && currentlogsData && (
                <div className="fixed inset-0 bg-gray-800/90 h-screen flex justify-center items-center z-120">
                    <div className="relative max-h-[80%] bg-white dark:bg-gray-700 shadow-xl p-3 rounded-lg w-full max-w-2xl overflow-x-hidden overflow-y-auto">
                        ...
                        <ViewlogsData logsData={currentlogsData} />
                    </div>
                </div>
            )} */}
...
export default withAuth(Logs, ["system-admin"], ["auth.view_permission", "ednant"]);
